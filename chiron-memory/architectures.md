...
## The fog layer is drawn from `vision.shadowPolygons`: one wedge per obstacle edge facing the viewer, all wound the same way, filled as a single non-zero path under the obstacles

What: `shadowPolygons(viewer, obstacles, far = SHADOW_FAR)` in src/sim/vision.ts returns, for every obstacle edge that faces the viewer, a 5-vertex wedge — the edge, its two ends and its midpoint pushed `far` (100 000) units along their rays from the viewer. Every wedge is ordered to positive winding, so `renderer.drawFog` puts them all in one path and fills once with the non-zero rule: overlapping shadows never double-darken. The fill is clipped to the arena square, painted after the floor and *before* obstacles (walls stay bright as cover), with `COLORS.fog` (`rgba(2,5,10,0.72)`); the lit floor/grid were brightened (`#16332a`) at the same time so the two zones read at a glance · Why: before this there was no visual fog at all — a hidden rival was simply not drawn — and playtesters could not tell where the fog was. Projecting edges (rather than computing a true visibility polygon by angular sweep) is a few lines, exactly matches `hasClearLine` for every point (vision.test.ts checks a sampled grid), and 14 obstacles × ≤2 front edges is trivial per frame · Where: src/sim/vision.ts (`shadowPolygons`, `SHADOW_FAR`), src/sim/vision.test.ts, src/renderer.ts (`drawFog`, `COLORS.arena/grid/fog`) · Learned: the midpoint vertex matters — a 360-unit edge seen from 25 units away spans ~165°, and a plain far chord would cut across the arena; points exactly on a wedge boundary (sight line grazing a corner) are legitimately either side, so the consistency test nudges and skips them.
...
## Fog of war previously had no visual overlay — it only suppressed drawing of hidden entiti…
...
What: window.arenaDebug exposes an `act(flags)` method (e.g. `act({dash:true, shield:true, bash:true, shot:true})`) to programmatically trigger one or more player abilities from the console/browser, alongside the previously known `step(n)` and `newGame(seed)`. · Why: needed to drive and screenshot the ability-cooldown sweep/countdown during HUD verification without manual input. · Where: window.arenaDebug (exposed by the game for scripted e2e testing). <!-- id: c0468463-20e6-4d0d-af7e-dabbc91f90ae-10 -->

## In the app's CSS grid layout, the left (build list) and right (status) side panels span b…
...
## In CONFIG.skills, dash/slash/shot/shield store cooldowns as per-level arrays, but bash.co…

What: In CONFIG.skills, dash/slash/shot/shield store cooldowns as per-level arrays, but bash.cooldownMs is a single scalar with no level progression · Why: bash has no leveled progression like the other four skills · Where: src/config.ts · Learned: a "halve every cooldown" pass must handle bash.cooldownMs separately from the four leveled arrays. <!-- id: 6343d600-6267-4701-81fd-7f5c7c65d540-0 -->
