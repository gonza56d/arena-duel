...
What: Ability cooldown is visualized with a CSS conic-gradient sweep on each `.ability` tile driven by a `--cd` custom property (fraction of cooldown remaining), a numeric countdown label, and a `ready`/`cooling`/`active` state class. While `.cooling`, the background SVG glyph is hidden (`.ability.cooling .ability-icon svg { visibility: hidden }`) so only the sweep + number read; the glyph shows in `ready`/`active`. The countdown sits in a solid dark pill (`.ability-countdown`: `inset:0; margin:auto; width/height:max-content` shrink-wraps and centres it, `background: rgba(5,6,10,0.85)`), hidden via `.ability:not(.cooling) .ability-countdown{display:none}` when not counting, so the digits stay legible over any part of the sweep · Why: three stacked layers (glyph + sweep + number) blurred together and made the remaining cooldown hard to read at a glance; dropping the glyph and giving the number a solid backing gives one clear visual channel while cooling · Where: src/style.css, src/hud.ts · Learned: this is presentation-only — `--cd`, cooldown timing, icon order and the state-class logic in hud.ts are untouched; the pill hugs the digits via the `inset:0 + margin:auto + max-content` absolute-centring trick, no extra DOM node needed. <!-- id: c0468463-20e6-4d0d-af7e-dabbc91f90ae-8 -->
