...
## v2 build persistence: `PUT /profile/build` replaces `configured_stats` whole; the Go validator hardcodes a catalog mirrored from src/config.ts

What: `PUT /profile/build` (protected) binds `{configured_stats: map[string]int}` and replaces the build in one atomic `$set` via `UserStore.SetConfiguredStats`; there is no partial update and no separate GET — the build rides on `GET /profile` as `configured_stats,omitempty`, so buildless users keep the exact v1 response shape. `validate.ConfiguredStats` is the Go mirror of `validateLoadout` (same rules, near-identical messages): the 10-stat catalog and per-stat max levels are hardcoded in `statMaxLevels` and `BuildPoints = 16` in light-backend/internal/validate/build.go · Why: the light backend is a separate Go module that cannot import src/config.ts, and v2 has no shared contract file — a hand-mirrored map with a "change together" comment was the cheapest correct option; full replacement (PUT) fits a 10-key build the client always knows entirely, and validation-before-write plus one `$set` makes partial writes impossible · Where: light-backend/internal/validate/build.go, internal/handlers/build.go, internal/store (SetConfiguredStats), internal/server/build_test.go · Learned: the work order said "26 stats" but the real catalog is the 10 leveled stats from the config tables (the known README contradiction); the backend validates against the 10.
